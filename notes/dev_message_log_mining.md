...
| 07-13 | ASR 幻听第四例："Okay. 要说什么？"——测试者没说 Okay；另注意同段 "知道。" 疑似把"不知道"截掉了否定词（语义反转，比幻听更危险） | Okay 也在 boost terms 里，进一步佐证 boost 词误命中；"不知道"→"知道" 这种句首截断请一并听音频确认 |
...
4. **母语不能写死成中文**（产品是国际化的）：现配置里 "The child's native language is Chinese" 应改成模板变量，由客户端按账号 locale 注入（如 {{nativeLanguage}} = "Chinese" / "Spanish" / "Korean"）。母语未知时用通用写法（"the child may mix English with their own language"）。指令里的反幻听例子同理，写成"孩子自己语言的第一个音节"而不是"中文的第一个音节"。
5. 小项：phrases 里 "My name is ..." 去掉字面 "..."；setting 里 "Expect single words..." 需要配一句反向约束（宁少勿多）。

建议替换的两段（英文原文，{{nativeLanguage}} 由客户端注入，可直接粘）：

```
- key: language
  value: >-
    Prioritize English transcription. The child's native language is {{nativeLanguage}}
    and they often mix it with English. Transcribe {{nativeLanguage}} speech as
    {{nativeLanguage}} and English speech as English. If a sound is too unclear to
    confidently assign to either language, omit it entirely — transcribe only what was
    clearly spoken. Never translate.
- key: instructions
  value: >-
    ...
    of a word in the child's own language is NOT a short English word ("yeah", "sure",
    "from"). When in doubt, output less, not more. Do not fix the child's grammar;
    write speech verbatim. Combine split or stretched word attempts into the full
    target word ("a... pple" -> "apple"). Keep single letter names and letter sounds
    as raw letters ("A", "B"). Retain playful noises and interjections exactly as
    heard: "meow", "wow", "uh-oh", "haha".
```

母语未知/未填时的兜底版 `language`：

```
    Prioritize English transcription. The child may mix English with their own native
    language. Transcribe each language as itself. If a sound is too unclear to
    confidently assign to a language, omit it entirely — transcribe only what was
    clearly spoken. Never translate.
```

另外顶层 `asr.language: en-US` 也一样：如果引擎支持按市场传主语言或多语言自动检测，应随账号 locale 走，不要全球写死 en-US + 中文 context。
