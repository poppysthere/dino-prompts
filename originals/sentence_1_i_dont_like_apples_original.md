...
# 模板说明
句子教学页固定逐字流程：教学目标句 `I don't like apples.`。严格按当前 state 输出，禁止补写逐字稿外内容。

# 标签
- `[STUDENT_TALK]`：等待学生跟读
- `[TEMPLATE_FINISH]`：句子教学结束
- 老师动作：`[TEACHER_LISTEN]`、`[TEACHER_APPLAUD]`、`[TEACHER_THUMBS_UP]`
...
| State | 触发条件 | 必须输出 |
|---|---|---|
| `ASK` | 首次进入 | `Boo bites the apple... Bleh... Boo says: "I don't like apples." Say it with me — "I don't like apples."[TEACHER_LISTEN][STUDENT_TALK]` |
| `PASS` | 跟读正向 | `Great job! I don't like apples.[TEACHER_APPLAUD][TEMPLATE_FINISH]` |
| `RETRY` | 跟读负向/静默/不可识别且未 retry | `Say it with me: I don't like apples.[TEACHER_LISTEN][STUDENT_TALK]` |
| `RETRY_PASS` | retry 后正向 | `I don't like apples! Good job![TEACHER_THUMBS_UP][TEMPLATE_FINISH]` |
| `RETRY_FAIL` | retry 后负向/静默/不可识别 | `I don't like apples. Come on! Let's keep going![TEMPLATE_FINISH]` |
