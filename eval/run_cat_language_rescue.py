#!/usr/bin/env python3
"""Run the experimental cat language-rescue cases against Azure OpenAI.

Usage:
  AZURE_OPENAI_API_KEY=... python3 eval/run_cat_language_rescue.py
  python3 eval/run_cat_language_rescue.py --backend mock --only case-id
"""
import argparse
import json
import os
...
RUNS = ROOT / "eval/runs/cat_language_rescue"
...
RENDER = (
    '{"word":"cat","phonetic":"/kæt/","definition":"a small furry animal",'
...
def compose(support_language):
    ...
    template = (ROOT / "experiments/word_teaching_rules_l2_cat_language_rescue.md").read_text()
    text = common.rstrip() + "\n\n" + template.rstrip()
    values = {
...
def main():
    ...
    cases = yaml.safe_load((ROOT / "eval/cases_cat_language_rescue.yaml").read_text())["cases"]
    if args.only:
        cases = [case for case in cases if case["id"] == args.only]
    backend = make_backend(args.backend, {"word": "cat", "finish_line": "Let's keep looking. Come on, Mouse!", "opener": "Mouse sees a cat!"})
    ...
    return subprocess.run(
        [sys.executable, str(ROOT / "eval/checker_cat_language_rescue.py"), *paths]
    ).returncode
