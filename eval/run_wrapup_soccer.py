#!/usr/bin/env python3
"""Run the 足球课 wrap-up battery against the Forge debug API.

Usage:
  FORGE_TOKEN=... python3 eval/run_wrapup_soccer.py [--model gpt-5.4-mini] [--only case-id]
"""
import argparse
import json
import os
...
RUNS = ROOT / "eval/runs/wrapup_soccer"
...
RENDER = ("words: ['Goal!', 'team', 'Come on!']  "
          "theme: soccer, World Cup festival lesson.")
STEP_VALUE = {"pre_video": "pre-video", "post_video": "post-video"}
DEFAULT_NAME = "tom"
...
def compose(step: str, name: str) -> str:
    ...
    tmpl = (ROOT / "prompts/festival/wrapup_teaching_rules_l1_soccer.md").read_text()
    text = common.rstrip() + "\n\n" + tmpl.rstrip()
    for k, val in {
...
def main():
    ...
    battery = yaml.safe_load((ROOT / "eval/cases_wrapup_soccer.yaml").read_text())
    ...
    sys.exit(subprocess.run(
        [sys.executable, str(ROOT / "eval/checker_wrapup_soccer.py"), *paths]).returncode)
