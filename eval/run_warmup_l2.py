#!/usr/bin/env python3
"""Run the L2 warm-up (lesson0 state machine) battery against the Forge debug API.

Drives the path_a / path_b groups of eval/cases_warmup.yaml against
common_l2 + warmup_l2_lesson0. The legacy l1 / l1_broken groups target the
frozen warmup_teaching_rules_l1l2.md and are not run here.

Usage:
  FORGE_TOKEN=... python3 eval/run_warmup_l2.py [--model gpt-5.4-mini] [--only case-id]
"""
import argparse
import json
import os
...
# (group -> is_first_meet, prompt name, checker name)
GROUPS = {
    "path_a": (True, "heidi", "heidi"),
    "path_b": (False, "nina", "nina"),
}


def compose(is_first_meet: bool, name: str, role: str = ROLE) -> str:
    common = (ROOT / "prompts/l2/common_teaching_simple_rules_l2.md").read_text()
    tmpl = (ROOT / "prompts/l2/warmup_teaching_rules_l2_lesson0.md").read_text()
    text = common.rstrip() + "\n\n" + tmpl.rstrip()
    for k, val in {
...
def run_case(backend, group, case):
    first_meet, prompt_name, checker_name = GROUPS[group]
    system = compose(first_meet, prompt_name, case.get("role", ROLE))
...
def main():
    ...
    battery = yaml.safe_load((ROOT / "eval/cases_warmup.yaml").read_text())
