#!/usr/bin/env python3
"""Run the L1 V2 pre-A1 lesson battery through Prompt Forge.
...
PROMPT_DIR = ROOT / "prompts/l1-v2"
CASES_PATH = ROOT / "eval/cases_l1_v2.yaml"
...
def case_expectations(family, case):
    forbidden = list(case.get("forbid_phrases", []))
    if FAMILIES[family]["stage"] == "word":
        forbidden.extend([
            r"starts with [ABJ](?:[.!?]|$)",
            r"play with [ABJ](?:[.!?]|$)",
            r"letter sound",
...
def main():
    ...
    stages = set(args.stage or ("leadin", "word", "wrapup"))
    selected = [
        (family, case)
    ...
    if args.resume:
        ...
        run_dir = ROOT / "eval/runs" / f"l1_v2_{args.model}_{stamp}"
        run_dir.mkdir(parents=True, exist_ok=False)
        report = {
            "model": args.model,
            "forge_picker_id": CATALOG_IDS.get(args.model, args.model),
            "lesson": "L1 V2 lead-in -> word practice -> wrap-up",
    ...
    for family, case in selected:
        ...
        # Reapply current assertions when resuming. A checker calibration must
        # not be hidden by stale expectation metadata stored in an old case.
        transcript.update(case_expectations(family, case))
