#!/usr/bin/env python3
"""Run the experimental horse language-rescue cases against Azure OpenAI."""
import argparse
import json
import os
import pathlib
...
RUNS = ROOT / "eval/runs/horse_language_rescue"
...
RENDER = (
    '{"word":"horse","phonetic":"/hɔːrs/","definition":"a big animal you can ride",'
...
def compose(support_language):
    ...
    template = (ROOT / "experiments/word_teaching_rules_l2_horse_language_rescue.md").read_text()
    text = common.rstrip() + "\n\n" + template.rstrip()
    values = {
...
def main():
    ...
    cases = yaml.safe_load((ROOT / "eval/cases_horse_language_rescue.yaml").read_text())["cases"]
    if args.only:
        cases = [case for case in cases if case["id"] == args.only]
    backend = make_backend(
        args.backend,
        {"word": "horse", "finish_line": "Let's go find out!", "opener": "Mouse sees a horse!"},
    )
    ...
    return subprocess.run(
        [sys.executable, str(ROOT / "eval/checker_horse_language_rescue.py"), *paths]
    ).returncode
